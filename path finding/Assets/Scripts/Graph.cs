	...
	public virtual Path GetShortestPath ( Node start, Node end )
	{
		...
		while ( unvisited.Count != 0 )
		{
			...
			for ( int i = 0; i < current.connections.Count; i++ )
			{
				...
				float length = Vector3.Distance ( current.transform.position, neighbor.transform.position ) * (float)Mathf.Abs( heightRepulsion * (current.transform.position.y - neighbor.transform.position.y));
